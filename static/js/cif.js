...
async function loadPDBML(data, noCnT) {
...
async function loadCIF(data, noCnT) {
  var parser = new CIFparser();
  parser.parse(data);
...
function cleanJSON_withDict(data, cifdic) {
...
// stream api
async function streamMMJSON(k, jso, fp) {
  await fp.write(`"${k}":{`);
...
async function streamCIF(data, settings, fp) {
  settings = settings || {};
